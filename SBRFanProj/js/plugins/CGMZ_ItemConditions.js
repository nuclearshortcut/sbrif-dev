 ...
 * rename the js file.
 * --------------------------Latest Version------------------------------------
 * Hi all, this latest version removed skill conditions, as these were moved to
 * the related plugin [CGMZ] Skill Conditions. The reason for this change is
 * to make it easier to understand what each plugin does.
 *
 * This update added switch and variable conditions to items. These will also
 * be included in [CGMZ] Skill Conditions. You can now restrict items from use
 * if a switch is OFF or a variable's value is less than 1.
 *
 * Version 1.2.0
 * - Added switch conditions
 * - Added variable conditions
 * - Split skill conditions into new plugin
 *
...
CGMZ.Versions["Item Conditions"] = "1.2.0";
CGMZ.ItemConditions = {};
...
CGMZ_Temp.prototype.createPluginData = function() {
	...
	for(const item of CGMZ.ItemConditions.ItemRestrictions) {
		...
		const switches = CGMZ_Utils.parseJSON(itemObj["Switches"], [], "[CGMZ] Item Conditions", `Invalid JSON in item id: ${id}'s switch restriction parameter.`).map(switchId => Number(switchId));
		const variables = CGMZ_Utils.parseJSON(itemObj["Variables"], [], "[CGMZ] Item Conditions", `Invalid JSON in item id: ${id}'s variable restriction parameter.`).map(varId => Number(varId));
		let professions = [];
		...
		this._itemRestrictions[id] = {maps: maps, troops: troops, professions: professions, switches: switches, variables: variables};
	}
};
//-----------------------------------------------------------------------------
